...
// Chat - Updated for new conversation structure
...
export const listConversations = async (limit = 20, skip = 0, isActive = null, token) => {
  ...
  return response.data; // Returns: { conversations: [...], total: ..., limit: ..., skip: ... }
};
...
export const archiveConversation = async (conversationId, token) => {
  ...
  return response.data; // Returns: { success: boolean, message: string }
};
 ...
 */
export const createConversation = async (token, title = null) => {
  ...
  return response.data; // Returns: { success: boolean, conversation_id: string, title: string, created_at: string }
};
...
/**
 * Rename conversation placeholder (backend not implemented)
 */
